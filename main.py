...
if __name__ == "__main__":
    ...
    print(
        "Temps d'exécution avec l'heuristique de Hamming: %s secondes"
        % elapsed_time
    )

    # On peut aussi utiliser l'heuristique de Manhattan
    start_time = time.time()
    taquin_manhattan = TaquinAStar(1, verbose=False)
    taquin_manhattan.solve()
    elapsed_time = time.time() - start_time
    print(
        "Temps d'exécution avec l'heuristique de Manhattan: %s secondes"
        % elapsed_time
    )
